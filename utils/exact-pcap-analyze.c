...
//        }
        ...
        if(trace_pkt_count == 1)
        {
            *trace_start = pkt_now;
            windowstartns = 0;
            prev_pkt_ts_ns = tsps_to_double_ns(&pkt_now);
        ...
        if( window_ns > options.window_ms * 1000 * 1000)
        {
            //Output stats here
        ...
        //const double ipg_ns = nowoffsetns - prev_pkt_end_ns;

        //printf("now=%li.%012li now=%.2lf, prev=%.2lf, ipg_ns=%.3lfns\n", pkt_now.secs, pkt_now.psecs, nowoffsetns, prev_pkt_ts_ns, ipg_ns);
...
int main(int argc, char** argv)
{
    ...
    trace_pkt_count = load_trace(expcap, &trace_start, &trace_stop);
...
//    ch_log_info("Packet rates are in the range [%0.2fpps,%0.2fpps] [%0.2fMpps,%0.2fMpps]\n", stats.minpps, stats.max_pps, stats.minwind_rate_pps/1000/1000, stats.max_pps/1000/1000);
//    ch_log_info("#########################################################\n");
